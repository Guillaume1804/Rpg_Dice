...
export type QuickStandardDieOptions = {
  modifier?: number;
};

export type LoadSavedGroupIntoDraftParams = {
  group: {
    id: string;
    name: string;
    rule_id?: string | null;
...
function shouldSplitEntryDie(die: DraftDie, availableRules: RuleRow[]) {
  const kind = getDraftDieRuleKind(die, availableRules);

  return kind === "single_check" || kind === "d20";
}

function getDraftDieRuleKind(
  die: DraftDie,
  availableRules: RuleRow[],
): string | null {
  if (die.rule_temp?.kind) return die.rule_temp.kind;

  if (die.rule_id) {
    return availableRules.find((rule) => rule.id === die.rule_id)?.kind ?? null;
  }
...
export function useQuickRollDraft({
  ...
  function addQuickStandardDie(
    sides: number,
    options: QuickStandardDieOptions = {},
  ) {
    const modifier = Number.isFinite(options.modifier ?? 0)
      ? (options.modifier ?? 0)
      : 0;
    ...
    setDraftGroups((prev) => {
      ...
      let targetGroup =
        next.find(
          (group) =>
            group.name === "Jet libre" && !group.rule_id && !group.rule_temp,
        ) ?? null;
  ...
  function addDieLineToDraftGroup(groupId: string, sides: number) {
    setDraftGroups((prev) =>
      prev.map((group) =>
        group.id === groupId
          ? {
  ...
  function loadSavedGroupIntoDraft(
    params: LoadSavedGroupIntoDraftParams,
  ): string {
    const draftGroupId = `draft-action-${params.group.id}-${Date.now()}`;

    const nextGroup: DraftGroupState = {
      id: draftGroupId,
      name: params.draftName ?? params.group.name,
      rule_id: params.group.rule_id ?? null,
      rule_temp: null,
      dice: params.dice.map((die) => ({
        sides: die.sides,
        qty: die.qty,
  ...
  function addQuickPresetDie(
    ...
    setDraftGroups((prev) => {
      ...
      let targetGroup = selectedDraftGroupId
        ? (next.find((group) => group.id === selectedDraftGroupId) ?? null)
        : null;
      ...
      resolvedGroupId = targetGroup.id;
      ...
      return next.map((group) => {
        ...
        return {
          ...
          // Important :
          // On nettoie l'ancien comportement de groupe pour éviter que les lignes
          // sans comportement héritent d'une règle globale.
          rule_id: null,
          rule_temp: null,
  ...
  function applyPresetToDraftDie(
    groupId: string,
    index: number,
    preset: QuickPresetSelection,
  ) {
    setDraftGroups((prev) =>
      prev.map((group) => {
        ...
        return {
          ...
          // Dans le nouveau Jet préparé, les comportements configurés depuis
          // une ligne restent sur cette ligne.
          rule_id: null,
          rule_temp: null,
  ...
  function adjustDraftDieQty(groupId: string, index: number, delta: number) {
    setDraftGroups((prev) =>
      prev.map((group) => {
        if (group.id !== groupId) return group;
        ...
        const shouldSplit = shouldSplitEntryDie(targetDie, availableRules);
        ...
        nextDice.splice(
          index,
          1,
          ...Array.from({ length: nextQty }, () => ({ ...baseDie })),
        );
  ...
  function toggleDraftDieSign(groupId: string, index: number) {
    setDraftGroups((prev) =>
      prev.map((group) => {
        if (group.id !== groupId) return group;
  ...
  function adjustDraftDieModifier(
    groupId: string,
    index: number,
    delta: number,
  ) {
    setDraftGroups((prev) =>
      prev.map((group) => {
        ...
        const currentModifier = Number.isFinite(targetDie.modifier ?? 0)
          ? (targetDie.modifier ?? 0)
          : 0;

        const nextModifier = Math.max(
          -99,
          Math.min(99, currentModifier + delta),
        );
  ...
  async function rollDraft(): Promise<GroupRollResult[] | null> {
  ...
  async function rollSingleDraftGroup(
    groupId: string,
  ...
  function clearDraftGroup(groupId: string) {
    ...
    setDraftResults((prev) =>
      prev.filter((result) => result.groupId !== groupId),
    );
