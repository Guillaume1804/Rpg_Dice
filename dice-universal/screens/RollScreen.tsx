...
function getRuleSummaryFromTempRule(rule: DraftTempRuleSummary | null | undefined) {
  if (!rule) return null;
  ...
  if (behaviorKey === "single_check" || rule.kind === "single_check") {
    ...
    if (critSuccess) {
      parts.push(`Critique ${critSuccess}`);
    }

    const critFailure = formatNumberList(params.crit_failure_faces);
    if (critFailure) {
      parts.push(`Échec critique ${critFailure}`);
    }

    return parts.join(" · ");
  }

  if (behaviorKey === "threshold_degrees" || rule.kind === "threshold_degrees") {
  ...
  if (behaviorKey === "success_pool" || rule.kind === "success_pool") {
    ...
    if (failFaces) {
      parts.push(`Complication sur ${failFaces}`);
    }

    if (params.glitch_rule === "ones_gt_successes") {
      parts.push("si échecs > succès");
    }

    if (params.glitch_rule === "ones_gte_successes") {
      parts.push("si échecs ≥ succès");
    }
  ...
  if (behaviorKey === "custom_pipeline" || rule.kind === "pipeline") {
    ...
    for (const step of steps) {
      ...
      if (step.op === "keep_highest") {
        parts.push(`Garde ${step.n} meilleurs`);
      }

      if (step.op === "keep_lowest") {
        parts.push(`Garde ${step.n} plus faibles`);
      }

      if (step.op === "drop_highest") {
        parts.push(`Retire ${step.n} meilleurs`);
      }
    ...
    if (complicationFaces) {
      parts.push(`Complication ${complicationFaces}`);
    }
...
function getRuleSummaryFromRuleId(
...
function getDraftGroupBehaviorSummary(
...
function formatDraftGroupDiceLabel(
  group: DraftGroupSummary | null,
  rulesMap: RuleNameMap,
) {
